<div class="static my-5 p-5 sm:pl-10 rounded-full shadow-md border border-gray-200  bg-gray-100 text-gray-500 rounded" x-data="{open:false,other:false}">
    <div class="" x-on:click="open=!open,other=false" wire:click="resetErrors">{{__("ui.statement.placeholder")}}</div>
    <div class="bg-gray-900 opacity-80 fixed inset-0" x-show="open" x-on:click="open=!open">    </div>
    <div class="absolute border rounded p-4 bg-white top-10 inset-x-10 sm:inset-x-1/4 sm:top-32" style="display: none;" x-init={open:false,other:false} x-show="open">
