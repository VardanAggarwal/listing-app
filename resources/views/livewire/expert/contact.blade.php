<div  class="static" x-data="{form:false,type:@entangle('type'),href:@entangle('href')}">
    ...
    <div class="flex justify-center gap-2 text-green-500">
        @if($call)
        <a x-on:click="                mixpanel.track('Call Clicked',{'profile_id':{{$profile->id}},'number':'{{$call}}'}); form=true; type='call'; href='tel:{{$call}}'" class="flex items-center gap-1"><i class="text-2xl fas fa-phone-alt"></i>{{__('Call')}}</a>
        @endif
        @if($whatsapp)
        <a  x-on:click="                mixpanel.track('Whatsapp Clicked',{'profile_id':{{$profile->id}},'number':'{{$whatsapp}}'}); form=true; type='whatsapp';href='https://wa.me/{{Str::remove('+',$whatsapp)}}?text={{__('ui.contact_message',['url'=>'https://app.seedsaversclub.com/profiles/'.$profile->id])}}'" class="ml-4 flex items-center gap-1"><i class="text-3xl fab fa-whatsapp"></i>{{__('Whatsapp')}}</a>
        @endif
    </div>
    @endunless
    <template x-if="form">
        <div class="fixed bg-white inset-x-10 inset-y-20 z-50 sm:inset-x-20 p-4 opacity-100" @mouseleave="form=false">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              @foreach($services as $service)
                <span class="{{in_array($service,$selected['service'])?'bg-green-300':''}} border p-2" wire:click="toggleSelected('service')"><i class="fas fa-{{in_array($service,array_keys($service_types))?$service_types[$service]:$service_types['others']}}"></i> <span class="text-xs">{{__(in_array($service,array_keys($service_types))?'ui.expert.services.'.$service:$service)}}</span></span>
              @endforeach
            </div>
            @unless($login)
