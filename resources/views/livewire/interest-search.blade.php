<div class="py-8 max-w-7xl mx-4 sm:mx-auto">
    <input type="text" placeholder="Search" wire:model="query" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm w-full">
    @if($results)
    <div class="flex flex-wrap mt-5 gap-2">
        @foreach($results as $result)
            <div class="mt-5  p-2 text-xs sm:text-sm rounded-lg shadow border {{in_array($result->id,$selected)?'bg-green-300':''}}" wire:click="toggleSelected({{$result->id}})">
                {{$result->name}}
            </div>
        @endforeach
    </div>
        ...
        @if($results->hasMorePages())
          <x-jet-button wire:click.prevent="loadMore" class="mt-4 text-center">{{__('Load More')}}</x-jet-button>
        @endif
        </div>
    @endif
