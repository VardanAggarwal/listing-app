<div class="bg-white p-5 mx-auto max-w-min">
    ...
    <tr class="text-lg"><th>{{ucwords($key)}}</td></th>
        @foreach($value as $item)
            <tr class="">
                <td class="w-32"><a class="" href="\{{Str::plural(Str::replace('App\\Models\\','',$item->interestable_type))}}\{{$item->interestable_id}}">{{$item->interestable->title}}{{$item->interestable->name}}</a></td>
                @if($item->others)
