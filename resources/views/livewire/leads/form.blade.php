<div class="">
    @unless($form||$success)
    <div>
      <div class="w-full pt-12 px-4 pb-4 bg-gradient-to-br from-lime-200 to-zinc-50">
        <h1 class="text-3xl mb-4">{{__('ui.leads.form.title')}}</h1>
        <span class="text-gray-700">{{__('ui.leads.form.subtitle')}}</span>
    ...
    @endunless
    @if($form)
    <div class="px-4" x-init="window.scrollTo(0, 0); mixpanel.track('Order Form Viewed')">
        ...
        <x-jet-button class="border rounded-full p-2 bg-gray-900 text-white" wire:click="submit" x-on:click="mixpanel.track('Order Submitted');        mixpanel.register({'Phone Number': '{{$profile['contact_number']}}','Pincode':'{{$profile['pincode']}}'});">{{__('ui.leads.form.submit_button')}}</x-jet-button>
        </div>
    </div>
    @endif
    @if($success)
    <div class="px-4 grid grid-cols-1 justify-items-center" x-init="setTimeout(() => {window.location.href='/'}, 1000)">
      <span class="text-xl text-green-900">{{__('ui.leads.form.submit_message')}}</span>
      <div class="mt-4 flex justify-center">
      <a href="/"><x-jet-button class="border rounded-full p-2 bg-gray-900 text-white">{{__('ui.leads.form.redirect_button')}}</x-jet-button></a>
      </div>
    </div>
    @endif
</div>