<div>
    ...
    @unless($version==0)
        <div class="flex gap-4 text-green-500" x-init="">
            ...
            <a href="https://wa.me/{{Str::remove('+',$number)}}?text={{__('ui.contact_message')}}" x-on:click="                mixpanel.track('Whatsapp Clicked',{'profile_id':{{$profile->id}},'number':'{{$profile->contact_number}}'})" class="ml-4 flex items-center gap-1"><i class="text-3xl fab fa-whatsapp"></i>{{__('Whatsapp')}}</a>
        </div>
    @endunless
</div>